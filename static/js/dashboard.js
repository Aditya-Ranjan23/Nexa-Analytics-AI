...
/* =========================================
   Global Notification System (Phase 8.1)
   ========================================= */
class NotificationManager {
    ...
    getIcon(type) {
        switch (type) {
            ...
            case 'progress': return 'fa-solid fa-circle-notch fa-spin';
            default: return 'fa-solid fa-bell';
        }
    }

    show(id, type, title, message, progress = null) {
        let el;
        let isNew = false;
        ...
        if (this.activeNotifications.has(id)) {
            el = this.activeNotifications.get(id);
            // Update type if changed
            el.className = `saas-notification show ${type === 'error' ? 'error' : ''}`;
            el.querySelector('.notification-icon').innerHTML = `<i class="${iconClass}"></i>`;
        } else {
            isNew = true;
            el = document.createElement('div');
            el.className = `saas-notification ${type === 'error' ? 'error' : ''}`;
            el.innerHTML = `
        ...
        // Update content
        el.querySelector('.notification-title').textContent = title;
        el.querySelector('.notification-message').textContent = message;

        // Update progress if applicable
        const pBar = el.querySelector('.notification-progress-bar');
        if (progress !== null && type === 'progress') {
            pBar.style.display = 'block';
            el.querySelector('.notification-progress-fill').style.width = `${progress}%`;
        } else {
...
class JobPoller {
    ...
    stop() {
        if (this.interval && this.activeJobs.size === 0) {
    ...
    poll() {
        ...
        fetch('/api/jobs/')
            ...
            .catch(err => console.error("Job polling failed:", err));
    }

    updateJob(job) {
        if (job.status === 'running' || job.status === 'queued') {
            let stage = job.current_stage || "Processing...";
            Notifier.show(job.id, 'progress', 'Background Task', stage, job.progress);
        } else if (job.status === 'completed') {
            Notifier.show(job.id, 'success', 'Task Complete', 'Data preparation finished successfully.');
            this.activeJobs.delete(job.id);
            if (typeof refreshDashboardState === 'function') {
                refreshDashboardState();
            }
            if (typeof loadDataTab === 'function') {
                loadDataTab();
            }
        } else if (job.status === 'failed') {
            Notifier.show(job.id, 'error', 'Task Failed', job.error_message || 'An error occurred during processing.');
            this.activeJobs.delete(job.id);
        } else if (job.status === 'cancelled') {
            Notifier.show(job.id, 'warning', 'Task Cancelled', 'The background task was cancelled.');
            this.activeJobs.delete(job.id);
        }
    }
}

const JobTracker = new JobPoller();

function patchUploadForms() {
    ...
    window.fetch = async function() {
        ...
        if (response.status === 202) {
            ...
            cloned.json().then(data => {
                if (data.job_id) {
                    JobTracker.track(data.job_id);
                    Notifier.show(data.job_id, 'progress', 'Upload Queued', 'Your file is uploaded and queued for processing.', 0);
                }
            }).catch(e => {});
        }
        return response;
